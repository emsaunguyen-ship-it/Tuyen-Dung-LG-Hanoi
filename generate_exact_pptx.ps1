# PowerShell Script to generate a 2-Slide PowerPoint Presentation containing ALL 4 detailed bullet points in each box
# Slide 1: Tiếng Việt (Chính xác từng từ theo yêu cầu)
# Slide 2: English (Exact 1-to-1 Translation)
...
function Build-Exact-Slide($slideData, $slideIndex) {
    ...
    # 3. Main Headline
    $headBox = $slide.Shapes.AddTextbox(1, 40, 46, 880, 46)
    ...
    # 4. Split-Screen Layout (2 Equal Cards: Width = 426 pt, Height = 372 pt)
    $cardWidth = 426
    $cardHeight = 372
    $cardTop = 102
    $left1 = 40
    ...
    # Card 1 Header Pill
    $c1Header = $slide.Shapes.AddShape(1, $left1 + 10, $cardTop + 10, $cardWidth - 20, 28)
    $c1Header.Fill.Solid()
    ...
    # Card 1 Body Text - Assign ALL bullet items at once so none get overwritten!
    $c1Body = $slide.Shapes.AddTextbox(1, $left1 + 10, $cardTop + 42, $cardWidth - 20, $cardHeight - 50)
    $t1 = $c1Body.TextFrame
    ...
    # Card 2 Header Pill
    $c2Header = $slide.Shapes.AddShape(1, $left2 + 10, $cardTop + 10, $cardWidth - 20, 28)
    $c2Header.Fill.Solid()
    ...
    # Card 2 Body Text - Assign ALL bullet items at once so none get overwritten!
    $c2Body = $slide.Shapes.AddTextbox(1, $left2 + 10, $cardTop + 42, $cardWidth - 20, $cardHeight - 50)
    $t2 = $c2Body.TextFrame
...
Build-Exact-Slide $exactData.vi 1
Build-Exact-Slide $exactData.en 2
...
Write-Host "Successfully generated PowerPoint slide with ALL 4 BULLET POINTS INCLUDED at: $outputPath"
